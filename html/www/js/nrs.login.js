...
var NRS = (function(NRS, $, undefined) {
  ...
  NRS.showLoginScreen = function() {
    ...
    $('#account_phrase_custom_panel')
      .find(':input:not(:button):not([type=submit])')
      .val('');
  ...
  NRS.registerAccount = function() {
    ...
    if (typeof PassPhraseGenerator == 'undefined') {
      $.when($.getScript('js/crypto/passphrasegenerator.js'))
        .done(function() {
          $loading.hide();
          $loaded.show();
  ...
  $('#generator_paper_wallet_link').click(function(e) {
    e.preventDefault();
    NRS.printPaperWallet(
      $('#account_phrase_generator_panel')
        .find('.step_2 textarea')
        .val()
    );
  });
        ...
        if (account != '') {
          $('#login_account').append(
            $('<li></li>')
              ...
              .append($('<button data-dismiss="modal" class="close" type="button">×</button>').attr('onClick', "NRS.removeAccount('" + account + "')"))
          );
        }
      });
  ...
  NRS.switchAccount = function(account) {
    ...
    $(
      '#account_balance, #account_balance_sidebar, #account_nr_assets, #account_assets_balance, #account_silver_balance, #account_currencies_balance, #account_nr_currencies, #account_purchase_count, #account_pending_sale_count, #account_completed_sale_count, #account_message_count, #account_alias_count'
    ).html('0');
    $('#id_search')
      .find('input[name=q]')
      .val('');
    $('.nav__list-sublevel ul')
      .not('.pre-header__list')
      .remove();
  ...
  NRS.getBonus = function() {
    let pack = { account: NRS.account, accountRS: NRS.accountRS };
    let bonusServer = 'https://bonus.icbcoin.com/bonus';
    $.get(bonusServer, pack, resp => {
      console.log('bonus RESP', resp);
      if (resp === 'Ok') {
        $('#bonus_state').text($.t('bonus_send'));
        $('#signup_bonus').hide();
      } else {
        $('#bonus_state').text($.t('bonus_only_new_account'));
      }
    });
  };
  ...
  NRS.login = function(isPassphraseLogin, id, callback, options) {
    ...
    NRS.logConsole(
      'login isPassphraseLogin = ' +
        isPassphraseLogin +
        ', isAccountSwitch = ' +
        options.isAccountSwitch +
        ', isSavedPassphrase = ' +
        options.isSavedPassphrase +
        ', isPreventLoginToNewAccount = ' +
        options.isPreventLoginToNewAccount
    );
    ...
    NRS.sendRequest('getBlockchainStatus', {}, function(response) {
      ...
      NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
        ...
        if (!NRS.account) {
          $.growl(
            $.t('error_find_account_id', {
              accountRS: data && data.account ? String(data.account).escapeHTML() : '',
            }),
            {
              type: 'danger',
              offset: 10,
              ...
              $.each(accounts, function(index, account) {
                if (account != '') {
                  $('#account_id_dropdown')
                    .find('.dropdown-menu')
                    .append(
                      $("<li class='switchAccount'></li>").append(
                        $('<a></a>')
                          .attr('href', '#')
  ...
  $('#logout_button_container').on('show.bs.dropdown', function() {
    if (NRS.forgingStatus != NRS.constants.FORGING) {
      $(this)
        .find("[data-i18n='logout_stop_forging']")
        .hide();
    }
  });
