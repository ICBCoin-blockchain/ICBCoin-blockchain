...
var NRS = (function(NRS, $, undefined) {
  ...
  NRS.handleIncomingTransactions = function(transactions, confirmedTransactionIds) {
    ...
    if (NRS.currentPage == 'peers') {
      ...
      NRS.currentPage == 'transactions' &&
      $('#transactions_type_navi')
        .find('li.active a')
        .attr('data-transaction-type') == 'unconfirmed'
    ) {
  ...
  NRS.getTransactionRowHTML = function(t, actions, decimals, isScheduled) {
    ...
    if (t.attachment.quantityQNT === undefined) {
      quantityCoin = symbolMinus + (t.amountNQT / Math.pow(10, 8)).toFixed() + '<img class="coin" src="img/coin_gold.svg" alt="Gold coin">';
      // quantityCoin = Number.parseInt(quantityCoin);
      // quantityCoin.toFixed();
    } else {
      quantityCoin = t.attachment.quantityQNT / Math.pow(10, 6) + '<img class="coin" src="img/coin_silver.svg" alt="Silver coin">';
    }
    var html = '';
    ...
    if (isScheduled) {
      ...
      html += '<span>';
      html += NRS.formatTimestamp(t.timestamp) + '</span>';
    }

    html += '</td>';
    html += '<td>';
    html += '<span>' + transactionType + '</span>';
    html += '</td>';
    html += "<td class='text-right'>" + quantityCoin + '</td>'; // + formattedAmount +
    html += "<td class='text-right'>" + formattedFee + '<img class="coin" src="img/coin_gold.svg" alt="Gold coin">' + '</td>';

    let sendTypeAddress;
    let operationIconType;
    if (NRS.getAccountLink(t, 'sender') == '/' && t.type == 2) {
      sendTypeAddress = 'Asset Exchange';
      operationIconType = '<svg class="icon-transfer"><use xlink:href="img/sprite.svg#transfer"></use></svg>';
    } else {
      sendTypeAddress = NRS.getAccountLink(t, 'sender');
      if (NRS.getAccountAddress(t, 'sender') === String(NRS.accountRS).escapeHTML()) {
        operationIconType = '<svg class="icon-transfer"><use xlink:href="img/sprite.svg#transfer"></use></svg>';
      } else {
        operationIconType = '<svg class="icon-replenishment"><use xlink:href="img/sprite.svg#replenishment"></use></svg>';
      }
    }
    html += "<td style='vertical-align:middle;'>" + operationIconType;
    html += (NRS.getAccountLink(t, 'recipient') == '/' && t.type == 2 ? 'Asset Exchange' : '<span>' + NRS.getAccountAddress(t, 'recipient')) + '</span>' + '</td>';
    if (!isScheduled) {
      html += "<td class='text-right'>" + (t.confirmed ? NRS.getBlockLink(t.height, null, true) : '-') + '</td>';
      html += "<td class='confirmations text-right'>";
  ...
  NRS.buildTransactionsTypeNavi = function() {
    ...
    html += 'data-i18n="[data-content]all">';
    ...
    html += 'data-i18n="[data-content]unconfirmed_account">';
    ...
    html += 'data-i18n="[data-content]unconfirmed_everyone">';
  ...
  NRS.buildTransactionsSubTypeNavi = function() {
    ...
    var typeIndex = $('#transactions_type_navi')
      .find('li.active a')
      .attr('data-transaction-type');
  ...
  NRS.pages.transactions = function(callback, subpage) {
    ...
    var selectedSubType = $('#transactions_sub_type_navi')
      .find('li.active a')
      .attr('data-transaction-sub-type');
    if (!selectedSubType) {
      selectedSubType = '';
    }
  ...
  NRS.setup.transactions = function() {
    ...
    var options = {
      id: sidebarId,
      titleHTML: '<svg><use xlink:href="img/sprite.svg#overview"></use></svg> <span data-i18n="dashboard">Dashboard</span>',
      page: 'dashboard',
      desiredPosition: 10,
    };
    ...
    // NRS.appendMenuItemToTSMenuItem(sidebarId, options);
    options = {
      titleHTML: '<svg><use xlink:href="img/sprite.svg#transaction-history"></use></svg> <span data-i18n="my_transactions">My Transactions</span>',
      type: 'PAGE',
      page: 'transactions',
    };
    NRS.appendMenuItemToTSMenuItem(sidebarId, options);
