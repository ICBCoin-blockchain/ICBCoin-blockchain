  ...
  function getPeerServicesLabel(services) {
    ...
    for (var i = 0; i < services.length; i++) {
      ...
      servicesText +=
        ...
        $.t(services[i].toLowerCase() + '_service') +
        "'>" +
        shortcut +
        '</a>';
      ...
      function(response) {
        if (response.peers && response.peers.length) {
          ...
          for (var i = 0; i < response.peers.length; i++) {
            ...
            var versionToCompare =
              !NRS.isTestNet && NRS.nrsVersion ? NRS.nrsVersion.versionNr : NRS.state.version;
            ...
            rows +=
              peer.state == 1
                ? "<svg class='round-check'><use xlink:href='img/sprite.svg#round-check'></use></svg>"
                : "<svg class='round-error'><use xlink:href='img/sprite.svg#round-error'></use></svg>";
            rows +=
              (peer.announcedAddress ? NRS.getPeerSpan(peer.announcedAddress) : $.t('unknown')) +
              '</td>';
            rows +=
              "<td class='text-right'" +
              (peer.weight > 0 ? " style='font-weight:bold'" : '') +
              '>' +
              NRS.formatWeight(peer.weight) +
              '</td>';
            rows += '<td class="text-right">' + NRS.formatVolume(peer.downloadedVolume) + '</td>';
            rows += '<td class="text-right">' + NRS.formatVolume(peer.uploadedVolume) + '</td>';
            rows += '<td><span>';
            rows +=
              (peer.application && peer.version
                ? NRS.escapeRespStr(peer.application) + ' ' + NRS.escapeRespStr(peer.version)
                : '?') + '</label></td>';
            ...
            if (NRS.needsAdminPassword) {
              rows +=
                "data-toggle='modal' data-target='#connect_peer_modal' data-peer='" +
                NRS.escapeRespStr(peer.announcedAddress) +
                "'>";
            } else {
              rows +=
                'onClick=\'NRS.connectPeer("' + NRS.escapeRespStr(peer.announcedAddress) + '");\'>';
            }
            rows += $.t('connect') + '</a>';
            rows += "<a class='btn btn-xs btn-default' href='#' ";
            rows +=
              "data-toggle='modal' data-target='#blacklist_peer_modal' data-peer='" +
              NRS.escapeRespStr(peer.announcedAddress) +
              "'>" +
              $.t('blacklist') +
              '</a>';
          ...
          $('#peers_uploaded_volume')
            .html(NRS.formatVolume(uploaded))
            .removeClass('loading_dots');
          $('#peers_downloaded_volume')
            .html(NRS.formatVolume(downloaded))
            .removeClass('loading_dots');
          $('#peers_connected')
            .html(connected)
            .removeClass('loading_dots');
          $('#peers_up_to_date')
            .html(upToDate + '/' + activePeers)
            .removeClass('loading_dots');
