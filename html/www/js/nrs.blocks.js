...
var NRS = (function(NRS, $) {
  ...
  NRS.updateDashboardLastBlock = function(block) {
    console.log(NRS);
    if (NRS.accountInfo.unconfirmedAssetBalances !== undefined) {
      ...
      $('#account_balance_silver').html(NRS.formatQuantity(NRS.accountInfo.assetBalances[0].balanceQNT, 6));
    }
    $('#nrs_current_block_time')
      .empty()
      .append(NRS.formatTimestamp(block.timestamp));
    $('.nrs_current_block')
      .empty()
      .append(NRS.escapeRespStr(block.height));
  };
  ...
  NRS.incoming.updateDashboardBlocks = function(newBlocksCount) {
    ...
    $('#dashboard_table')
      .find('tr.confirmed td.confirmations')
      .each(function() {
  ...
  NRS.pages.blocks = function() {
    if (NRS.blocksPageType == 'forged_blocks') {
      ...
      $('#blocks_transactions_per_hour_box, #blocks_generation_time_box,#blocks_avg_fee_per_block,#blocks_avg_amount_per_block').hide();
      NRS.sendRequest(
        'getAccountBlocks+',
        {
          account: NRS.account,
      ...
      $('#blocks_transactions_per_hour_box, #blocks_generation_time_box,#blocks_avg_fee_per_block,#blocks_avg_amount_per_block').show();
  ...
  NRS.blocksPageLoaded = function(blocks) {
    ...
    for (var i = 0; i < blocks.length; i++) {
      ...
      rows +=
        '<tr>' +
        '<td>' +
        NRS.formatTimestamp(block.timestamp) +
        '</td>' +
        '<td>' +
        NRS.getAccountAddress(block, 'generator') +
        '</td>' +
        '<td class="text-right">' +
        NRS.formatAmount(block.totalAmountNQT) +
        '<img class="coin" src="img/coin_gold.svg" alt="Gold coin">' +
        '</td>' +
        '<td class="text-right">' +
        NRS.formatAmount(block.totalFeeNQT) +
        '<img class="coin" src="img/coin_gold.svg" alt="Gold coin">' +
        '</td>' +
        '<td class="text-right">' +
        NRS.formatAmount(block.numberOfTransactions) +
        '</td>' +
        "<td class='text-center'><span data-block='" +
        NRS.escapeRespStr(block.height) +
        "' data-blockid='" +
        NRS.escapeRespStr(block.block) +
        "' class='block show_block_modal_action'" +
        (block.numberOfTransactions > 0 ? " style='font-weight:bold'" : '') +
        '>' +
        NRS.escapeRespStr(block.height) +
        '</span></td>' +
        '<td class="text-right">' +
        NRS.formatVolume(block.payloadLength) +
        '</td>' +
        '<td class="text-right">' +
        NRS.baseTargetPercent(block).pad(4) +
        ' %</td>' +
        '</tr>';
    }
    ...
    if (NRS.blocksPageType == 'forged_blocks') {
      NRS.sendRequest(
        ...
        function(response) {
          if (response.numberOfBlocks && response.numberOfBlocks > 0) {
            $('#forged_blocks_total')
              .html(response.numberOfBlocks)
              .removeClass('loading_dots');
            var avgFee = new Big(NRS.accountInfo.forgedBalanceNQT)
              .div(response.numberOfBlocks)
              .div(new Big('100000000'))
              .toFixed(2);
            $('#blocks_average_fee')
              .html(NRS.formatStyledAmount(NRS.convertToNQT(avgFee)))
              .removeClass('loading_dots');
          } else {
            $('#forged_blocks_total')
              .html(0)
      ...
      $('#forged_fees_total')
        .html(NRS.formatStyledAmount(NRS.accountInfo.forgedBalanceNQT))
        .removeClass('loading_dots');
      ...
      var averageAmount = 0;
      if (blocks.length) {
        averageFee = new Big(totalFees.toString())
          .div(new Big('100000000'))
          .div(new Big(String(blocks.length)))
          .toFixed(2);
        averageAmount = new Big(totalAmount.toString())
          .div(new Big('100000000'))
          .div(new Big(String(blocks.length)))
          .toFixed(2);
      }
      ...
      if (time == 0) {
        $('#blocks_transactions_per_hour')
          .html('0')
          .removeClass('loading_dots');
      } else {
        $('#blocks_transactions_per_hour')
          .html(Math.round((totalTransactions / (time / 60)) * 60))
          .removeClass('loading_dots');
      }
      $('#blocks_average_generation_time')
        .html(Math.round(time / NRS.itemsPerPage) + 's')
        .removeClass('loading_dots');
      $('#blocks_average_fee')
        .html(NRS.formatStyledAmount(averageFee))
        .removeClass('loading_dots');
      blocksAverageAmount
        .parent()
        .parent()
  ...
  NRS.blockchainDownloadingMessage = function() {
    if (NRS.state.apiProxy) {
      return (
        $.t(NRS.state.isLightClient ? 'status_light_client_proxy' : 'status_blockchain_downloading_proxy', { peer: NRS.getPeerLink(NRS.state.apiProxyPeer) }) +
        " <a href='#' class='btn btn-xs' data-toggle='modal' data-target='#client_status_modal'>" +
        $.t('proxy_info_link') +
        '</a>'
      );
