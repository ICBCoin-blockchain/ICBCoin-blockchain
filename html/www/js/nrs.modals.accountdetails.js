...
var NRS = (function(NRS, $, undefined) {
  ...
  accountDetailsModal.on('show.bs.modal', function(e) {
    ...
    if (NRS.accountInfo.errorCode && NRS.accountInfo.errorCode != 5) {
      ...
      if(NRS.accountInfo.assetBalances === undefined){
        $('#account_balance_silver').html('0');
      }
      if (NRS.accountInfo.errorCode && NRS.accountInfo.errorCode == 5) {
        $('#account_balance_balance, #account_balance_unconfirmed_balance, #account_balance_effective_balance, #account_balance_guaranteed_balance, #account_balance_forged_balance').html(
          '0 ' + NRS.constants.COIN_SYMBOL
        );
        ...
        $('#account_balance_unconfirmed_balance').html(NRS.formatAmount(new BigInteger(NRS.accountInfo.unconfirmedBalanceNQT)));
        $('#account_balance_effective_balance').html(NRS.formatAmount(NRS.accountInfo.effectiveBalanceNXT));
        $('#account_balance_guaranteed_balance').html(NRS.formatAmount(new BigInteger(NRS.accountInfo.guaranteedBalanceNQT)));
        $('#account_balance_forged_balance').html(NRS.formatAmount(new BigInteger(NRS.accountInfo.forgedBalanceNQT)));
  ...
  accountDetailsModal.on('hidden.bs.modal', function() {
    $(this)
      .find('.account_details_modal_content')
      .hide();
    $(this)
