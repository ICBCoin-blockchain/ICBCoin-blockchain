...
var NRS = (function(NRS, $, undefined) {
  ...
  function initImpl() {
    ...
    loadConstantsPromise.then(function() {
      ...
      var getStatePromise = new Promise(function(resolve) {
        ...
        NRS.sendRequest(
          ...
          function(response) {
            ...
            var customLoginWarningDiv = $('.login-warning');
      ...
      getStatePromise.then(function() {
        ...
        if (!(navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1) && navigator.userAgent.indexOf('JavaFX') == -1) {
        ...
        $('#dashboard_table, #transactions_table')
          .on('mouseenter', 'td.confirmations', function() {
            $(this).popover('show');
          })
  ...
  NRS.connectionError = function(errorDescription, errorCode) {
    ...
    var msg = $.t('error_server_connect', { url: NRS.getRequestPath() }) + (errorDescription ? ' ' + NRS.escapeRespStr(errorDescription) : '');
    $.growl(msg, {
      type: 'danger',
      offset: 10,
    });
  ...
  $('.js-logo, .nav__list, .nav__list-sublevel').on('click', 'a', function(e, data) {
    e.preventDefault();

    /* If link open modal window */
    if ($(this).data('toggle') === 'modal') {
      return;
    }

    /* Submenu add active class */
    let submenuID = $(this)
      .closest('li')
      .attr('id');
    ...
    if (
      $(this)
        .closest('li')
        .attr('class') !== 'sm_treeview_submenu' &&
      $(this)
        .closest('li')
        .attr('class') !== 'sm_treeview_submenu active'
    ) {
      if (
        $(this)
          .children('span')
        ...
        let activeTimer = setInterval(function() {
          $('#asset_exchange_sidebar_content a').click();
          if ($('#asset_exchange_sidebar_content a').length > 0) {
      ...
      $('.sm_treeview_submenu').removeClass('active');
      $(this)
        .closest('li')
        .addClass('active');
      $('.treeview-menu[data-id="' + submenuID + '"]').addClass('active');
    } else {
      $('.sm_treeview_submenu').removeClass('active');
      $(this)
        .closest('li')
    ...
    // $('ul.nav__list > li.active').each(function(key, elem) {
    ...
    // $('ul.nav__list > li.sm_simple').removeClass('active');
    // if ($newActiveA.parent('li').hasClass('sm_simple')) {
    //   $newActiveA.parent('li').addClass('active');
    // }

    // $('ul.nav__list li.sm_treeview_submenu').removeClass('active');
    // if (
    //   $(this)
    //     .parent('li')
    //     .hasClass('sm_treeview_submenu')
    // ) {
    //   $(this)
    //     .closest('li')
    //     .addClass('active');
  ...
  NRS.goToPage = function(page, callback, subpage) {
    var $link = $('ul.nav__list a[data-page=' + page + ']');
  ...
  NRS.addPagination = function() {
    ...
    var prevHTML = '<span>';
    var firstHTML = '<span>';
    var currentHTML = '<span>';
    var nextHTML = '<span>';

    if (NRS.pageNumber > 1) {
      prevHTML += "<a class='pagination__btn' href='#' data-page='" + (NRS.pageNumber - 1) + "' title='" + $.t('previous') + "'>";
      prevHTML += $.t('previous') + '</a>';
    } else {
      prevHTML += '&nbsp;';
    }
    ...
    if (NRS.hasMorePages) {
      ...
      nextHTML += "<a class='pagination__btn' href='#' data-page='" + (NRS.pageNumber + 1) + "' title='" + $.t('next') + "'>";
      nextHTML += $.t('next') + '</a>';
    } else {
      if (NRS.pageNumber > 1) {
    ...
    NRS.sendRequest(
      ...
      function(response) {
        ...
        if (response.peers && response.peers.length) {
          ...
          connectedIndicator
            .find('span')
            .html($.t('Not Connected'))
            .attr('data-i18n', 'not_connected');
          connectedIndicator.show();
        }
      }
    );
    ...
    NRS.sendRequest(
      ...
      function(response) {
        ...
        if (response.errorCode) {
          ...
          $(
            '#account_balance, #account_balance_sidebar, #account_currencies_balance, #account_nr_currencies, #account_purchase_count, #account_pending_sale_count, #account_completed_sale_count, #account_message_count, #account_alias_count'
          ).html('0');
          NRS.updateDashboardMessage();
        } else {
          ...
          let exactBalanceClean = response.unconfirmedBalanceNQT;
          let exactBalanceFixed = NRS.formatStyledAmountWithoutAfterPoint(exactBalanceClean).replace(/\s/g, '');
          let exactBalanceLength = exactBalanceFixed.replace(',', '').length;
          let exactBalanceResult = [exactBalanceFixed, '.', ...exactBalanceClean.slice(exactBalanceLength, exactBalanceClean.length)].join('');

          let resultSilverBalance,fullBalance,fullSilverBalance;
          if(NRS.accountInfo.assetBalances !== undefined) {
            ...
            resultSilverBalance = (parseFloat(NRS.accountInfo.assetBalances[0].balanceQNT)/1000000).toString();
            ...
            fullSilverBalance = (parseFloat(NRS.accountInfo.assetBalances[0].balanceQNT)/1000000).toString();
            fullSilverBalance = fullSilverBalance.replace('.', ',')

            fullBalance = (parseFloat(response.unconfirmedBalanceNQT)/100000000).toString()
            fullBalance = fullBalance.replace('.', ',')
          } else {
            resultSilverBalance = 0;
            fullBalance = 0;
            fullSilverBalance = 0;
          }

          $('#account_assets_balance').html(resultSilverBalance);
          $('#account_balance_balance').html(fullBalance);
          $('#account_balance_silver').html(fullSilverBalance);
          ...
          if (NRS.isDisplayOptionalDashboardTiles()) {
            ...
            var showAssetDifference = !isAccountSwitch && (!NRS.downloadingBlockchain || (NRS.blocks && NRS.blocks[0] && NRS.state && NRS.state.time - NRS.blocks[0].timestamp < 60 * 60 * 24 * 7));
            ...
            if ((firstRun || isAccountSwitch) && response.assetBalances) {
              ...
              NRS.sendRequest(
                ...
                function(response) {
                  let btnSendAssetName = NRS.accountInfo.assetBalances[0].asset;
                  let btnSendAsset =
                    '<button class="pre-header__btn-send-ms pre-header__btn-send-asset btn_primary" style="min-height: 40px" type="button" data-toggle="modal" data-name="SILVER" data-target="#transfer_asset_modal" data-decimals="6" data-asset="' +
                    btnSendAssetName +
                    '" data-action="transfer_asset"><img src="img/coin_silver.svg" alt="Silver coin"><span>' +
                    $.t('send') +
                    '</span></button>';


                  if (+resultSilverBalance.replace(',', '.') > 0) {
                    $('.pre-header .nav__btn-group').prepend(btnSendAsset);
                  } else {
                    $('.pre-header__btn-send-asset').remove();
                  }
                  ...
                  if (response.trades && response.trades.length) {
                    ...
                    let accountBalanceSilverClean = assetBalances[0].balanceQNT;
                    let accountBalanceSilverSlice = assetBalances[0].balanceQNT.slice(0, -6);
                    let acountBalanceSilverLength = accountBalanceSilverSlice.length;
                    let acountBalanceSilverResult = [
                      ...accountBalanceSilverClean.slice(0, acountBalanceSilverLength),
                      '.',
                      ...accountBalanceSilverClean.slice(acountBalanceSilverLength, accountBalanceSilverClean.length),
                    ].join('');

                    let acountBalanceSilverResultFixed = [
                      ...accountBalanceSilverClean.slice(0, acountBalanceSilverLength),
                      '.',
                      ...accountBalanceSilverClean.slice(acountBalanceSilverLength, acountBalanceSilverLength + 2),
                    ].join('');

                    let assetBalanceFormatted = NRS.formatQuantity(assetBalances[0].balanceQNT, 6);
                    ...
                    $('#account_assets_balance').html(resultSilverBalance);
                    $('#account_balance_silver').html(assetBalanceFormatted);
                    $('#account_silver_balance').html(NRS.formatStyledAmount(new Big(assetTotal).toFixed(8)));
                    $('#account_nr_assets').html(response.trades.length);
                  } else {
                    $('#account_assets_balance').html(0);
                    $('#account_silver_balance').html(0);
              ...
              function(response) {
                if (response.transactions && response.transactions.length) {
                  if (response.transactions.length > 99)
                    ...
                    $('#account_message_count')
                      .empty()
                      .append(response.transactions.length);
                } else {
                  $('#account_message_count')
                    .empty()
                    .append('0');
            ...
            NRS.sendRequest(
              ...
              function(response) {
                if (response.numberOfAliases != null) {
                  $('#account_alias_count')
                    .empty()
                    .append(response.numberOfAliases);
                }
              }
            );
              ...
              function(response) {
                if (response.numberOfPurchases != null) {
                  $('#account_purchase_count')
                    .empty()
                    .append(response.numberOfPurchases);
                }
              }
            );
            ...
            NRS.sendRequest(
              ...
              function(response) {
                if (response.purchases && response.purchases.length) {
                  $('#account_pending_sale_count')
                    .empty()
                    .append(response.purchases.length);
                } else {
                  $('#account_pending_sale_count')
                    .empty()
                    .append('0');
          ...
          if (response.name) {
            $('#account_name')
              .html(NRS.addEllipsis(NRS.escapeRespStr(response.name), 17))
        ...
        if (firstRun) {
          $(
            '#account_balance, #account_balance_sidebar, #account_assets_balance, #account_silver_balance, #account_nr_assets, #account_currencies_balance, #account_nr_currencies, #account_purchase_count, #account_pending_sale_count, #account_completed_sale_count, #account_message_count, #account_alias_count'
          ).removeClass('loading_dots');
        }
    ...
    if (NRS.accountInfo.errorCode) {
      if (NRS.accountInfo.errorCode == 5) {
        if (NRS.downloadingBlockchain && !(NRS.state && NRS.state.apiProxy) && !NRS.state.isLightClient) {
          if (NRS.newlyCreatedAccount) {
            ...
            //       '<br/><br/>' +
            //       NRS.blockchainDownloadingMessage() +
            //       '<br/><br/>'
            //   )
            //   .show();
          } else {
            $('#dashboard_message')
              .addClass('alert-success')
              .removeClass('alert-danger')
              .html(NRS.blockchainDownloadingMessage())
              .show();
          }
      ...
      if (NRS.downloadingBlockchain) {
        ...
        $('#dashboard_message')
          .addClass('alert-danger')
          .removeClass('alert-success')
          .html(warning + ' ' + $.t('public_key_actions'))
          .show();
  ...
  NRS.updateAccountLeasingStatus = function() {
    ...
    if (NRS.accountInfo.lessors) {
      ...
      for (var i = 0; i < NRS.accountInfo.lessorsRS.length; i++) {
        ...
        rows +=
          '<tr>' +
          '<td>' +
          NRS.getAccountLink({ lessorRS: lessor }, 'lessor') +
          '</td>' +
          '<td>' +
          NRS.escapeRespStr(lessorInfo.effectiveBalanceNXT) +
          '</td>' +
          '<td><label>' +
          String(blocksLeft).escapeHTML() +
          " <i class='fa fa-question-circle show_popover' data-toggle='tooltip' title='" +
          blocksLeftTooltip +
          "' data-placement='right' style='color:#4CAA6E'></i></label></td>" +
          '<td><label>' +
          String(nextLessee).escapeHTML() +
          " <i class='fa fa-question-circle show_popover' data-toggle='tooltip' title='" +
          nextTooltip +
          "' data-placement='right' style='color:#4CAA6E'></i></label></td>" +
          '</tr>';
    ...
    if (accountLeasingStatus) {
      $('#account_leasing_status')
        .html(accountLeasingStatus)
        .show();
    } else {
  ...
  NRS.updateBlockchainDownloadProgress = function() {
    ...
    if (NRS.state.isLightClient) {
      ...
      if (!blocksLeft || blocksLeft < parseInt(lastNumBlocks / 2)) {
        ...
        downloadingBlockchain.find('.db_progress_total .sr-only').html(
          $.t('percent_complete', {
      ...
      if (!blocksLeft || blocksLeft >= lastNumBlocks * 2 || NRS.state.lastBlockchainFeederHeight <= lastNumBlocks) {
        ...
        downloadingBlockchain.find('.db_progress_last .sr-only').html(
          $.t('percent_complete', {
