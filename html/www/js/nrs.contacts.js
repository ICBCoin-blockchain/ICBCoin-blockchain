...
var NRS = (function(NRS, $) {
    ...
    NRS.storageSelect('contacts', null, function(error, contacts) {
      ...
      if (contacts && contacts.length) {
        ...
        $.each(contacts, function(index, contact) {
          ...
          rows +=
            '<tr>' +
            "<td style='width: 15% '><a href='#' data-toggle='modal' data-target='#update_contact_modal' data-contact='" +
            String(contact.id).escapeHTML() +
            "'>" +
            contact.name.escapeHTML() +
            '</a></td>' +
            '<td style="width: 20%"><span>' +
            NRS.getAccountFormatted(contact, 'account') +
            '</span></td>' +
            '<td style="width: 18%">' +
            (contact.email ? contact.email.escapeHTML() : '-') +
            '</td>' +
            '<td style="width: 17%">' +
            contactDescription.escapeHTML() +
            '</td>' +
            "<td style='white-space:nowrap;width: 30%;'><div class='group-btn-box'>" +
            "<button class='btn__primary btn__primary--with-coin' type='button' data-toggle='modal' data-target='#send_money_modal' data-contact='" +
            String(contact.name).escapeHTML() +
            "'><img src='img/coin_gold.svg' alt='Gold coin'>" +
            $.t('send') +
            ' ' +
            NRS.constants.COIN_SYMBOL +
            '</button>&nbsp;' +
            "<button class='btn__dark btn__dark--with-icon' data-toggle='modal' data-target='#send_message_modal' data-contact='" +
            String(contact.name).escapeHTML() +
            "'><svg><use xlink:href='img/sprite.svg#message'></use></svg>" +
            $.t('message') +
            '</button>&nbsp;' +
            "<button class='btn__warning-border btn__warning-border--with-icon' data-toggle='modal' data-target='#delete_contact_modal' data-contact='" +
            String(contact.id).escapeHTML() +
      ...
      function(error, contacts) {
        if (contacts && contacts.length) {
          if (contacts[0].name == data.name) {
            $modal
              .find('.error_message')
              .html($.t('error_contact_name_exists'))
              .show();
          } else {
            $modal
              .find('.error_message')
              .html($.t('error_contact_account_id_exists'))
              .show();
          }
          $btn.button('reset');
          $modal.modal('unlock');
  ...
  NRS.setup.contacts = function() {
    NRS.addTreeviewSidebarMenuItem({
      id: 'sidebar_contacts',
      titleHTML: '<svg><use xlink:href="img/sprite.svg#contacts"></use></svg><span data-i18n="contacts">Contacts</span>',
      page: 'contacts',
      desiredPosition: 200,
    });
  };
        ...
        function(error, contacts) {
          if (contacts && contacts.length) {
            if (contacts[0].name == imported_contact.name) {
              $.growl(imported_contact.name + ' - ' + $.t('error_contact_name_exists'), {
                type: 'warning',
              }).show();
            } else {
              $.growl(imported_contact.account + ' - ' + $.t('error_contact_account_id_exists'), {
                type: 'warning',
              }).show();
            }
          } else {
