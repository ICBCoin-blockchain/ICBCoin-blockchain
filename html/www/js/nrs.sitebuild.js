...
var NRS = (function(NRS, $) {
  ...
  NRS.initModalUIElement = function($modal, selector, elementName, context) {
    var html = $(_modalUIElements)
      .filter('div#' + elementName)
      .html();
    var template = Handlebars.compile(html);
  ...
  function _appendToHeaderNav(menuHTML, id, desiredPosition) {
    if ($('#' + id).length == 0) {
      var inserted = false;
      var headerNavMenu = $('#header-nav'); //#sidebar_menu
      $.each(headerNavMenu.find('> li'), function(key, elem) {
      ...
      if (!inserted) {
        headerNavMenu.append(menuHTML);
      }
      headerNavMenu.find('[data-i18n]').i18n();
    }
  }
  ...
  NRS.addSimpleSidebarMenuItem = function(options) {
    ...
    _appendToHeaderNav(menuHTML, options['id'], options['desiredPosition']);
  };
  ...
  NRS.addTreeviewSidebarMenuItem = function(options) {
    ...
    menuHTML += '<a href="#" data-page="' + options['page'] + '">' + options['titleHTML'] + '</a>';
    menuHTML += '</li>';
    ...
    $('#pre-header-sub-menu').append('<ul class="treeview-menu ' + activeClass + '" data-id="' + options['id'] + '"></ul>');
  };
  ...
  NRS.appendMenuItemToTSMenuItem = function(itemId, options) {
    ...
    var parentMenu = $("ul[data-id='" + itemId + "']");
