...
var NRS = (function(NRS, $) {
  ...
  userInfoModal.on('hidden.bs.modal', function() {
    $(this)
      .find('.user_info_modal_content')
      .hide();
    $(this)
      .find('.user_info_modal_content table tbody')
      .empty();
    $(this)
      .find('.user_info_modal_content:not(.data-loading,.data-never-loading)')
        ...
        if (response.transactions && response.transactions.length) {
          ...
          for (var i = 0; i < response.transactions.length; i++) {
            ...
            rows +=
              '<tr>' +
              '<td>' +
              NRS.getTransactionLink(transaction.transaction, NRS.formatTimestamp(transaction.timestamp)) +
              '</td>' +
              '<td>' +
              transactionType +
              '</td>' +
              "<td class='numeric'>" +
              (!receiving && transaction.amount > 0 ? '-' : '') +
              '' +
              NRS.formatAmount(transaction.amount, false, false, amountDecimals) +
              '</td>' +
              "<td class='numeric'>" +
              NRS.formatAmount(transaction.fee, false, false, feeDecimals) +
              '</td>' +
              '<td>' +
              NRS.getAccountAddress(transaction, account) +
              '</td>' +
              '</tr>';
          }

          infoModalTransactionsTable
            .find('tbody')
            .empty()
            .append(rows);
      ...
      function(response) {
        ...
        if (response.aliases && response.aliases.length) {
          ...
          for (var i = 0; i < aliases.length; i++) {
            ...
            rows +=
              "<tr data-alias='" +
              NRS.escapeRespStr(String(alias.aliasName).toLowerCase()) +
              "'><td class='alias'>" +
              NRS.escapeRespStr(alias.aliasName) +
              "</td><td class='uri'>" +
              (alias.aliasURI.indexOf('http') === 0
                ? "<a href='" + NRS.escapeRespStr(alias.aliasURI) + "' target='_blank'>" + NRS.escapeRespStr(alias.aliasURI) + '</a>'
                : NRS.escapeRespStr(alias.aliasURI)) +
              '</td></tr>';
          }
        }
        var infoModalAliasesTable = $('#user_info_modal_aliases_table');
        infoModalAliasesTable
          .find('tbody')
          .empty()
          .append(rows);
      ...
      function(response) {
        ...
        if (response.goods && response.goods.length) {
          for (var i = 0; i < response.goods.length; i++) {
            ...
            rows +=
              "<tr><td><a href='#' data-goto-goods='" +
              NRS.escapeRespStr(good.goods) +
              "' data-seller='" +
              NRS.escapeRespStr(NRS.userInfoModal.user) +
              "'>" +
              NRS.escapeRespStr(good.name) +
              "</a></td><td class='numeric'>" +
              NRS.formatAmount(good.priceNQT, false, false, priceDecimals) +
              ' ' +
              NRS.constants.COIN_SYMBOL +
              "</td><td class='numeric'>" +
              NRS.format(good.quantity, false, quantityDecimals) +
              '</td></tr>';
          }
        }
        var infoModalMarketplaceTable = $('#user_info_modal_marketplace_table');
        infoModalMarketplaceTable
          .find('tbody')
          .empty()
          .append(rows);
        ...
        if (response.accountCurrencies && response.accountCurrencies.length) {
          for (var i = 0; i < response.accountCurrencies.length; i++) {
            ...
            rows +=
              '<tr>' +
              '<td>' +
              NRS.getTransactionLink(NRS.escapeRespStr(currency.currency), code) +
              '</td>' +
              '<td>' +
              currency.name +
              '</td>' +
              "<td class='numeric'>" +
              NRS.formatQuantity(currency.unconfirmedUnits, currency.decimals, false, unitsDecimals) +
              '</td>' +
              '</tr>';
          }
        }
        var infoModalCurrenciesTable = $('#user_info_modal_currencies_table');
        infoModalCurrenciesTable
          .find('tbody')
          .empty()
          .append(rows);
      ...
      function(response) {
        ...
        if (response.trades && response.trades.length) {
          ...
          for (var i = 0; i < trades.length; i++) {
            ...
            rows +=
              "<tr><td><a href='#' data-goto-asset='" +
              NRS.escapeRespStr(trades[i].asset) +
              "'>" +
              NRS.escapeRespStr(trades[i].name) +
              '</a></td><td>' +
              NRS.formatTimestamp(trades[i].timestamp) +
              "</td><td style='color:" +
              (type == 'buy' ? 'green' : 'red') +
              "'>" +
              $.t(type) +
              "</td><td class='numeric'>" +
              NRS.formatQuantity(trades[i].quantityQNT, trades[i].decimals, false, quantityDecimals) +
              "</td><td class='asset_price numeric'>" +
              NRS.formatOrderPricePerWholeQNT(trades[i].priceNQT, trades[i].decimals, priceDecimals) +
              "</td><td class='numeric' style='color:" +
              (type == 'buy' ? 'red' : 'green') +
              "'>" +
              NRS.formatAmount(trades[i].totalNQT, false, false, amountDecimals) +
              '</td></tr>';
          }
        }
        var infoModalTradeHistoryTable = $('#user_info_modal_trade_history_table');
        infoModalTradeHistoryTable
          .find('tbody')
          .empty()
          .append(rows);
  ...
  NRS.userInfoModal.assetsLoaded = function(assets) {
    ...
    for (var i = 0; i < assetArray.length; i++) {
      ...
      rows +=
        '<tr' +
        (asset.issued ? " class='asset_owner'" : '') +
        "><td><span data-goto-asset='" +
        NRS.escapeRespStr(asset.asset) +
        "'" +
        (asset.issued ? " style='font-weight:bold'" : '') +
        '>' +
        NRS.escapeRespStr(asset.name) +
        "</span></td><td class='quantity numeric'>" +
