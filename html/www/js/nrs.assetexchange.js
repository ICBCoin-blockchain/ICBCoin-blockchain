...
var NRS = (function(NRS, $, undefined) {
    ...
    if (viewingAsset) {
      NRS.saveAssetBookmarks(new Array(viewingAsset), function(newAssets) {
        ...
        NRS.loadAssetExchangeSidebar(function() {
          $('#asset_exchange_sidebar')
            .find('a[data-asset=' + newAssets[0].asset + ']')
            .addClass('active')
            .trigger('click');
        });
      });
  ...
  NRS.forms.addAssetBookmarkComplete = function(newAssets, submittedAssets) {
    ...
    if (newAssets.length == 0) {
      ...
      assetExchangeSidebar
        .find('a[data-asset=' + submittedAssets[0].asset + ']')
        .addClass('active')
        .trigger('click');
    } else {
      NRS.closeModal();
  ...
  NRS.loadAssetExchangeSidebar = function(callback) {
      ...
      if (asset.groupName.toLowerCase() != lastGroup) {
        ...
        if (asset.groupName) {
          ...
          rows +=
            ...
            NRS.unescapeRespStr(asset.groupName)
              .toUpperCase()
              .escapeHTML() +
            '</h4>';
    ...
    if (isSearch) {
      if (active && assetSearch.indexOf(active) != -1) {
        ...
        exchangeSidebar
          .find('a[data-asset=' + assetSearch[0] + ']')
          .addClass('active')
          .trigger('click');
      }
    } else if (active) {
  ...
  NRS.incoming.asset_exchange = function() {
    ...
    assetExchangeSidebar
      .find('a.list-group-item.owns_asset')
      .removeClass('owns_asset')
      .addClass('not_owns_asset');
    if (NRS.accountInfo.assetBalances) {
      $.each(NRS.accountInfo.assetBalances, function(key, assetBalance) {
        if (assetBalance.balanceQNT != '0') {
          $('#asset_exchange_sidebar')
            .find('a.list-group-item[data-asset=' + assetBalance.asset + ']')
            .addClass('owns_asset')
  ...
  NRS.loadAsset = function(asset, refresh) {
    ...
    if (!refresh) {
      ...
      $('#sell_asset_for_nxt').html(
        $.t('sell_asset_for_nxt', {
          assetName: NRS.escapeRespStr(asset.name) === 'SILVER' ? '<img src="img/coin_silver.svg" alt="Silver coin">' : NRS.escapeRespStr(asset.name),
          symbol: NRS.constants.COIN_SYMBOL === 'ICBC' ? '' : NRS.constants.COIN_SYMBOL,
        })
      );
      $('#buy_asset_with_nxt').html(
        $.t('buy_asset_with_nxt', {
          assetName: NRS.escapeRespStr(asset.name) === 'SILVER' ? '<img src="img/coin_silver.svg" alt="Silver coin">' : NRS.escapeRespStr(asset.name),
          symbol: NRS.constants.COIN_SYMBOL === 'ICBC' ? '' : NRS.constants.COIN_SYMBOL,
        })
      );
      ...
      assetExchangeAskOrdersTable
        .parent()
        .addClass('data-loading')
        .removeClass('data-empty');
      assetExchangeBidOrdersTable
        .parent()
        .addClass('data-loading')
        .removeClass('data-empty');
      assetExchangeTradeHistoryTable
        .parent()
        .addClass('data-loading')
        .removeClass('data-empty');
        ...
        function(response) {
          if (!response.errorCode) {
            if (
              ...
              response.accountRS != asset.accountRS ||
              response.decimals != asset.decimals ||
    ...
    if (NRS.accountInfo.unconfirmedAssetBalances) {
      for (var i = 0; i < NRS.accountInfo.unconfirmedAssetBalances.length; i++) {
        ...
        if (balance.asset == assetId) {
          ...
          let cleanSilverBalance = NRS.accountInfo.assetBalances[0].balanceQNT;
          let beginNumberSilver = NRS.accountInfo.assetBalances[0].balanceQNT.slice(0, -6);
          let resultSilverBalance = (parseFloat(NRS.accountInfo.assetBalances[0].balanceQNT)/1000000).toString();
          let dotPosition = resultSilverBalance.indexOf('.');
          resultSilverBalance = resultSilverBalance.slice(0, dotPosition + 3);
          resultSilverBalance= resultSilverBalance.replace('.', ',');
          $('#account_assets_balance').html(resultSilverBalance);
          $('#your_asset_balance').html(resultSilverBalance);

          if (balance.unconfirmedBalanceQNT == '0') {
            $('#sell_automatic_price')
              .addClass('zero')
              .removeClass('nonzero');
          } else {
            $('#sell_automatic_price')
    ...
    if (orders.length) {
      ...
      for (var i = 0; i < orders.length; i++) {
        ...
        rows +=
          ...
          className +
          "' data-transaction='" +
          NRS.escapeRespStr(order.order) +
          "' data-quantity='" +
          order.quantityQNT.toString().escapeHTML() +
          "' data-price='" +
          order.priceNQT.toString().escapeHTML() +
          "'>" +
          '<td>' +
          NRS.getAccountAddress(order, 'account', currentAsset.accountRS, 'asset_issuer') +
          '</td>' +
          "<td class='text-right'>" +
          NRS.formatQuantity(order.quantityQNT, currentAsset.decimals, false, quantityDecimals) +
          "<img class='coin' src='img/coin_silver.svg' alt='Silver coin'></td>" +
          "<td class='text-right'>" +
          NRS.formatOrderPricePerWholeQNT(order.priceNQT, currentAsset.decimals, priceDecimals) +
          "<img class='coin' src='img/coin_gold.svg' alt='Gold coin'></td>" +
          "<td class='text-right'>" +
          NRS.formatAmount(sum, false, false, amountDecimals) +
          "<img class='coin' src='img/coin_gold.svg' alt='Gold coin'></td>" +
          '</tr>';
      }
      $('#asset_exchange_' + type + '_orders_table tbody')
        .empty()
        .append(rows);
    } else {
  ...
  NRS.getAssetDividendHistory = function(assetId, table) {
    ...
    assetExchangeDividendHistoryTable
      .parent()
      .addClass('data-loading')
      .removeClass('data-empty');
    var options = {
      ...
      if (response.trades && response.trades.length) {
        ...
        for (var i = 0; i < trades.length; i++) {
          ...
          rows +=
            '<tr>' +
            '<td>' +
            NRS.formatTimestamp(trade.timestamp) +
            '</td>' +
            '<td>' +
            $.t(trade.tradeType) +
            '</td>' +
            "<td class='text-right'>" +
            NRS.formatQuantity(trade.quantityQNT, currentAsset.decimals, false, quantityDecimals) +
            "<img class='coin' src='img/coin_silver.svg' alt='Silver coin'></td>" +
            "<td class='text-right'>" +
            NRS.formatAmount(trade.totalNQT, false, false, amountDecimals) +
            "<img class='coin' src='img/coin_gold.svg' alt='Gold coin'></td>" +
            '<td>' +
            NRS.getAccountAddress(trade, 'buyer', currentAsset.accountRS, 'asset_issuer') +
            '</td>' +
            '<td>' +
            NRS.getAccountAddress(trade, 'seller', currentAsset.accountRS, 'asset_issuer') +
            '</td>' +
            '</tr>';
        }
        exchangeTradeHistoryTable
          .find('tbody')
          .empty()
          .append(rows);
  ...
  $('#asset_exchange_trade_history_type')
    .find('.btn')
    .click(function(e) {
      e.preventDefault();
  ...
  $('#asset_order_modal').on('show.bs.modal', function(e) {
    ...
    if (orderType === 'Buy') {
      btnText = $.t('buy_active');
    } else {
      btnText = $.t('sell_active');
    }

    $('#asset_order_modal_button')
      .html(btnText)
      .data('resetText', orderType + ' Asset');
  ...
  $('#asset_exchange_group_modal').on('hidden.bs.modal', function() {
    $('#asset_exchange_group_new_group_div')
      .val('')
      .hide();
  });
        ...
        if (response.trades && response.trades.length) {
          ...
          for (var i = 0; i < trades.length; i++) {
            ...
            rows +=
              ...
              '</td>' +
              "<td class='numeric'>" +
              NRS.formatQuantity(trade.quantityQNT, trade.decimals, false, quantityDecimals) +
              "<img class='coin' src='img/coin_silver.svg' alt='Silver coin'></td>" +
              "<td class='text-right'>" +
              NRS.formatAmount(trade.totalNQT, false, false, amountDecimals) +
              "<img class='coin' src='img/coin_gold.svg' alt='Gold coin'></td>" +
              '<td>' +
              NRS.getAccountAddress(trade, 'buyer') +
              '</td>' +
              '<td>' +
              NRS.getAccountAddress(trade, 'seller') +
              '</td>' +
              '</tr>';
          }
          NRS.dataLoaded(rows);
      ...
      function(response) {
        if (response.transfers && response.transfers.length) {
          ...
          for (var i = 0; i < transfers.length; i++) {
            ...
            let silver_coin;
            if (NRS.escapeRespStr(transfer.name) === 'SILVER') {
              silver_coin = '<img class="coin" src="img/coin_silver.svg" alt="Silver coin">';
            } else {
              silver_coin = '';
            }
            console.log();
            ...
            rows +=
              '<tr>' +
              '<td>' +
              NRS.getTransactionLink(transfer.assetTransfer) +
              '</td>' +
              '<td>' +
              silver_coin +
              '</td>' +
              '<td>' +
              NRS.formatTimestamp(transfer.timestamp) +
              '</td>' +
              "<td style='" +
              (type == 'receive' ? 'color:green' : 'color:red') +
              "' class='numeric'>" +
              NRS.formatQuantity(transfer.quantityQNT, transfer.decimals, false, quantityDecimals) +
              '</td>' +
              '<td>' +
              NRS.getAccountLink(transfer, 'recipient') +
              '</td>' +
              '<td>' +
              NRS.getAccountLink(transfer, 'sender') +
              '</td>' +
              '</tr>';
          }
          NRS.dataLoaded(rows);
        ...
        if (response.deletes && response.deletes.length) {
          ...
          for (var i = 0; i < deletes.length; i++) {
            ...
            rows +=
              '<tr>' +
              '<td>' +
              NRS.getTransactionLink(deletes[i].assetDelete) +
              '</td>' +
              "<td><a href='#' data-goto-asset='" +
              NRS.escapeRespStr(deletes[i].asset) +
              "'>" +
              NRS.escapeRespStr(deletes[i].name) +
              '</a></td>' +
              '<td>' +
              NRS.formatTimestamp(deletes[i].timestamp) +
              '</td>' +
              "<td class='numeric'>" +
              NRS.formatQuantity(deletes[i].quantityQNT, deletes[i].decimals, false, quantityDecimals) +
              '</td>' +
              '</tr>';
          }
          NRS.dataLoaded(rows);
  ...
  NRS.myAssetsPageLoaded = function(result) {
    ...
    for (var i = 0; i < result.assets.length; i++) {
      ...
      rows +=
        "<tr data-asset='" +
        NRS.escapeRespStr(asset.asset) +
        "'>" +
        "<td style='width: 10%'><span>" +
        NRS.escapeRespStr(asset.name) +
        '</span></td>' +
        "<td class='quantity numeric' style='width: 10%'>" +
        NRS.formatQuantity(asset.balanceQNT, asset.decimals, false, quantityDecimals) +
        '</td>' +
        "<td class='numeric' style='width: 15%'>" +
        NRS.formatQuantity(asset.quantityQNT, asset.decimals, false, totalDecimals) +
        '</td>' +
        "<td class='numeric' style='width: 10%'>" +
        percentageAsset +
        '%</td>' +
        "<td class='numeric' style='width: 15%'>" +
        (lowestAskOrder != -1 ? NRS.formatOrderPricePerWholeQNT(lowestAskOrder, asset.decimals, askDecimals) : '') +
        '</td>' +
        "<td class='numeric' style='width: 15%'>" +
        (highestBidOrder != -1 ? NRS.formatOrderPricePerWholeQNT(highestBidOrder, asset.decimals, bidDecimals) : '') +
        '</td>' +
        "<td style='width: 23%'><div class='group-btn-box'>" +
        "<button class='btn__primary btn__primary--with-coin' data-toggle='modal' data-target='#transfer_asset_modal' data-asset='" +
        NRS.escapeRespStr(asset.asset) +
        "' data-name='" +
        NRS.escapeRespStr(asset.name) +
        "' data-decimals='" +
        NRS.escapeRespStr(asset.decimals) +
        "' data-action='transfer_asset'>" +
        "<img src='img/coin_silver.svg' alt='Silver coin'>" +
        $.t('transfer') +
        '</button>' +
        "<button class='btn__warning-border btn__warning-border--with-icon' data-toggle='modal' data-target='#transfer_asset_modal' data-asset='" +
        NRS.escapeRespStr(asset.asset) +
        "' data-name='" +
        NRS.escapeRespStr(asset.name) +
        "' data-decimals='" +
        NRS.escapeRespStr(asset.decimals) +
        "' data-action='delete_shares'>" +
        "<svg><use xlink:href='img/sprite.svg#remove-history'></use></svg>" +
        $.t('delete_shares') +
        '</button>' +
        '</div></td>' +
        '</tr>';
      ...
      function(response) {
        ...
        if (response.accountAssets) {
          ...
          for (var i = 0; i < response.accountAssets.length; i++) {
            ...
            rows +=
              '<tr><td>' +
              NRS.getAccountLink(account, 'account', currentAsset.accountRS, 'asset_issuer') +
              '</td><td>' +
              NRS.formatQuantity(account.quantityQNT, currentAsset.decimals) +
              '</td><td>' +
              percentageAsset +
              '%</td></tr>';
          }
  ...
  $('#transfer_asset_modal').on('show.bs.modal', function(e) {
    ...
    if (confirmedBalance == unconfirmedBalance) {
      ...
      availableAssetsMessage =
        ...
        ' (' +
        NRS.formatQuantity(confirmedBalance, decimals) +
        ' ' +
        $.t('total_lowercase') +
        ')';
    }
  ...
  NRS.goToAsset = function(asset) {
    ...
    $('ul.sidebar-menu a[data-page=asset_exchange]')
      .last()
      .trigger('click', [
        {
          callback: function() {
  ...
  NRS.openOrdersLoaded = function(orders, type, callback) {
    ...
    for (var i = 0; i < orders.length; i++) {
      ...
      rows +=
        "<tr data-order='" +
        NRS.escapeRespStr(completeOrder.order) +
        "'><td><span>" +
        NRS.escapeRespStr(completeOrder.assetName) +
        "</span></td><td class='text-right'>" +
        NRS.formatQuantity(completeOrder.quantityQNT, completeOrder.decimals) +
        "<img class='coin' src='img/coin_silver.svg' alt='Silver coin'></td><td class='text-right'>" +
        NRS.formatOrderPricePerWholeQNT(completeOrder.priceNQT, completeOrder.decimals) +
        "<img class='coin' src='img/coin_gold.svg' alt='Gold coin'></td><td class='text-right'>" +
        NRS.formatAmount(completeOrder.totalNQT) +
        "<img class='coin' src='img/coin_gold.svg' alt='Gold coin'></td><td class='text-right cancel'><a href='#' data-toggle='modal' data-target='#cancel_order_modal' data-order='" +
        NRS.escapeRespStr(completeOrder.order) +
        "' data-type='" +
        type +
        "'>" +
  ...
  NRS.setup.asset_exchange = function() {
    ...
    var options = {
      id: sidebarId,
      titleHTML: '<svg><use xlink:href="img/sprite.svg#asset-exchange"></use></svg><span data-i18n="assets">Assets</span>',
      page: 'asset_exchange',
      desiredPosition: 30,
    ...
    options = {
      titleHTML: '<svg><use xlink:href="img/sprite.svg#my-active"></use></svg><span data-i18n="my_assets">My Assets</span></a>',
      type: 'PAGE',
      page: 'my_assets',
    };
    NRS.appendMenuItemToTSMenuItem(sidebarId, options);
    options = {
      titleHTML: '<svg><use xlink:href="img/sprite.svg#deal-history"></use></svg><span data-i18n="trade_history">Trade History</span></a>',
      type: 'PAGE',
      page: 'trade_history',
    };
    NRS.appendMenuItemToTSMenuItem(sidebarId, options);
    ...
    // NRS.appendMenuItemToTSMenuItem(sidebarId, options);
    options = {
      titleHTML: '<svg><use xlink:href="img/sprite.svg#open-orders"></use></svg><span data-i18n="open_orders">Open Orders</span>',
      type: 'PAGE',
      page: 'open_orders',
    };
    NRS.appendMenuItemToTSMenuItem(sidebarId, options);
    options = {
      titleHTML: '<svg><use xlink:href="img/sprite.svg#remove-history"></use></svg><span data-i18n="delete_history">Delete History</span>',
      type: 'PAGE',
