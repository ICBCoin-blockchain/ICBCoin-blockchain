...
var NRS = (function(NRS, $) {
  ...
  NRS.setup.messages = function() {
    NRS.addTreeviewSidebarMenuItem({
      id: 'sidebar_messages',
      titleHTML: '<svg><use xlink:href="img/sprite.svg#message"></use></svg><span data-i18n="messages">Messages</span>',
      page: 'messages',
      desiredPosition: 90,
  ...
  NRS.jsondata.messages = function(response) {
    ...
    if (decoded.extra == 'pruned') {
      retrieveAction =
        "<a href='#' class='btn btn-xs' data-toggle='modal' data-transaction='" +
        response.transaction +
        "' data-hash='" +
        decoded.hash +
        "' data-target='#retrieve_message_modal'>" +
        $.t('retrieve') +
        '</a>';
    }
    var shareAction = '';
  ...
  function renderMyMessagesTable() {
    ...
    NRS.sendRequest('getBlockchainTransactions+', params, function(response) {
      ...
      response.transactions.forEach(function(transactionsJson) {
  ...
  function displayMessageSidebar(callback) {
    ...
    for (var i = 0; i < sortedMessages.length; i++) {
      ...
      rows +=
        "<a href='#' class='list-group-item' data-account='" +
        NRS.getAccountFormatted(sortedMessage, 'user') +
        "' data-account-id='" +
        NRS.getAccountFormatted(sortedMessage.user) +
        "' " +
        extra +
        ...
        NRS.getAccountTitle(sortedMessage, 'user') +
        '</h4>' +
        "<p class='list-group-item-text'>" +
        NRS.formatTimestamp(sortedMessage.timestamp) +
        '</p></a>';
    }
    messagesSidebar.empty().append(rows);
    if (activeAccount) {
      messagesSidebar
        .find('a[data-account=' + activeAccount + ']')
        .addClass('active')
        .trigger('click');
    }
    NRS.pageLoaded(callback);
  ...
  function getMessage(message) {
    ...
    if (!message.attachment) {
      ...
      NRS.sendRequest(
        'getPrunableMessage',
        { transaction: message.transaction, retrieve: 'false' },
        function(response) {
    ...
    if (!$.isEmptyObject(decoded)) {
      ...
      decoded.message = NRS.addEllipsis(
      ...
      if (decoded.extra == 'to_decrypt') {
        decoded.format = "<svg><use xlink:href='img/sprite.svg#eye-close'></use></svg>";
      } else if (decoded.extra == 'decrypted') {
        decoded.format += "<svg class='eye'><use xlink:href='img/sprite.svg#eye'></use></svg>";
  ...
  $('#messages_sidebar').on('click', 'a', function(e) {
    ...
    if (messages) {
      for (var i = 0; i < messages.length; i++) {
        ...
        if (messageClass === 'from') {
          messageDate = '<div class="chat__message-time">' + NRS.formatTimestamp(messages[i].timestamp) + '</div>';
        } else {
          messageDate = '';
        }
        ...
        output += '<div class="chat__message-' + messageClass + '"><p>' + decoded.format + decoded.message + sharedKeyTag + '</p>' + messageDate + '</div>';
      }
    }
    output += '</div>';
    $('#message_details')
  ...
  $('#messages_sidebar_update_context').on('click', 'a', function(e) {
    ...
    if (option == 'update_contact') {
      ...
      $('#send_money_recipient')
        .val(NRS.selectedContext.data('contact'))
        .trigger('blur');
      $('#send_money_modal').modal('show');
    }
  });
  ...
  $('body').on('click', 'a[data-goto-messages-account]', function(e) {
    ...
    NRS.goToPage('messages', function() {
      $('#message_sidebar')
        .find('a[data-account=' + account + ']')
        .trigger('click');
    });
  });
        ...
        NRS.addUnconfirmedTransaction(response.transaction, function(alreadyProcessed) {
          if (!alreadyProcessed) {
            $('#message_details')
              .find('dl.chat')
              .append(
                "<dd class='to tentative" +
                  (data.encryptedMessageData ? ' decrypted' : '') +
                  "'><p>" +
                  (data.encryptedMessageData ? "<i class='fa fa-lock'></i> " : '') +
                  (!data['_extra'].message ? $.t('message_empty') : String(data['_extra'].message).escapeHTML()) +
                  '</p></dd>'
              );
            var splitter = $('#messages_page').find('.content-splitter-right-inner');
            splitter.scrollTop(splitter[0].scrollHeight);
    ...
    if (!(data['_extra'] && data['_extra'].convertedAccount)) {
      $.growl(
        $.t('success_message_sent') +
          " <a href='#' data-account='" +
          NRS.getAccountFormatted(data, 'recipient') +
  ...
  $('#message_details').on('click', 'div.to_decrypt', function() {
  ...
  $('#shared_key_modal').on('show.bs.modal', function(e) {
    ...
    if (NRS.state.apiProxy) {
      ...
      url += '?account=' + NRS.accountRS + '&modal=transaction_info_modal' + '&transaction=' + transaction + '&sharedKey=' + sharedKey;
  ...
  $('#messages_decrypt_modal, #transaction_info_modal').on('show.bs.modal', function() {
    $(
      '#messages_decrypt_password, #messages_decrypt_remember_password, #messages_decrypt_shared_key, ' + '#decrypt_note_form_password, #decrypt_note_remember_password, #decrypt_note_form_shared_key'
    ).prop('disabled', false);
  });
