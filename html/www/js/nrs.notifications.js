...
var NRS = (function(NRS, $, undefined) {
  NRS.updateNotificationUI = function() {
    ...
    if (totalCount > 0) {
      ...
      $menuItem.find('.nm_inner_total').addClass('active');

      var $popoverHeaderBox = '<div class="popover-header">' + $.t('notification_title', 'Notification') + '</div>';
      var $markReadDiv = $("<div class='popover-footer'></div>");
      var $markReadLink = $("<a href='#'>" + $.t('notifications_mark_as_read', 'Mark all as read') + '</a>');
      $markReadLink.click(function(e) {
        e.preventDefault();
        NRS.resetNotificationState();
        $menuItem.popover('hide');
      });
      $popoverItem.prepend($popoverHeaderBox);
      $markReadLink.appendTo($markReadDiv);
      $popoverItem.append($markReadDiv);
      document.title = $.t('app_title') + ' (' + String(totalCount) + ')';
    } else {
      $menuItem.find('.nm_inner_subtype').css('backgroundColor', '#337ab7');
      $menuItem.find('.nm_inner_total').removeClass('active');
      var html = '';
    ...
    var template = '<div class="popover" style="min-width:320px;"><div class="popover-inner">';
  ...
  NRS.setShufflingNotifications = function() {
    NRS.sendRequest('getAllShufflings', {}, function(response) {
