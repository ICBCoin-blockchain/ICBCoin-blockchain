...
var NRS = (function(NRS, $) {
  ...
  NRS.forms.stopForgingComplete = function(response, data) {
    ...
    if (response.foundAndStopped || (response.stopped && response.stopped > 0)) {
      ...
      if (!response.forgersCount || response.forgersCount == 0) {
        ...
        forgingIndicator
          .find('span')
          .html($.t(NRS.constants.NOT_FORGING))
          .attr('data-i18n', 'forging');
      }
  ...
  forgingIndicator.click(function(e) {
    ...
    if (NRS.state.isLightClient) {
      ...
      // Если публичный ключ отсутствует
    ...
    } else if (forgingRealStatus === 'forging') {
      //NRS.isAccountForging прежнее условие
  ...
  forgingIndicator.hover(function() {
  ...
  NRS.updateForgingStatus = function(secretPhrase) {
    ...
    // Обновления форджинга здесь
    forgingIndicator
      .find('span')
      .html($.t(status))
      .attr('data-i18n', status);
