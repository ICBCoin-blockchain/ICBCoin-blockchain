...
var NRS = (function(NRS, $, undefined) {
  ...
  NRS.calculatePercentage = function(a, b, rounding_mode) {
    ...
    if (rounding_mode != undefined) {
      // Rounding mode from Big.js
  ...
  NRS.getAccountAddress = function(object, accountKey, accountRef, title, showAccountRS, clazz) {
    var accountRS;
    ...
    return String(accountRS).escapeHTML();
  };
  ...
  NRS.getAccountLink = function(object, accountKey, accountRef, title, showAccountRS, clazz) {
    ...
    return "<a href='#' data-user='" + String(accountRS).escapeHTML() + "' class='show_account_modal_action user-info" + clazz + "'>" + accountTitle + '</a>';
  };
  ...
  NRS.getTransactionLink = function(id, text, isEscapedText) {
    ...
    return "<span class='show_transaction_modal_action'>" + (isEscapedText ? text : String(text).escapeHTML()) + '</span>';
  };
  ...
  NRS.getBlockLink = function(height, text, isEscapedText) {
    ...
    return '<span>' + (isEscapedText ? text : String(text).escapeHTML()) + '</span>';
  };
  ...
  NRS.getPeerLink = function(address) {
    ...
    return '<span>' + String(address).escapeHTML() + '</span>';
  };
    ...
    if ($el.length) {
      ...
      try {
        ...
        $el
          .find('tbody')
          .empty()
          .append('<tr><td>Error processing table data: ' + e.message + '</td></tr>');
      }
    }
  ...
  NRS.createInfoTable = function(data, fixed) {
    ...
    for (var key in data) {
      ...
      if (/_formatted_html$/i.test(key)) {
        ...
        (key == 'quantity' ||
          key == 'units' ||
          key == 'initial_buy_supply' ||
          key == 'initial_sell_supply' ||
          key == 'total_buy_limit' ||
          key == 'total_sell_limit' ||
          key == 'units_exchanged' ||
          key == 'total_exchanged' ||
          key == 'initial_units' ||
          key == 'reserve_units' ||
      ...
      } else if (key == 'request_processing_time') {
        /* Skip from displaying request processing time */
        continue;
      } else {
      ...
      rows +=
        ...
        ":</td><td style='width:90%;word-break:break-all'>" +
        value +
        '</td></tr>';
    }
  ...
  NRS.formatStyledAmount = function(strAmount, round) {
    ...
    if (amount.length == 2) {
      return amount[0] + "<span class='residue'>" + locale.decimal + amount[1] + '</span>';
    } else {
      return amount[0];
    }
  };

  NRS.formatStyledAmountWithoutAfterPoint = function(strAmount, round) {
    var locale = NRS.getLocale();
    var amount = NRS.formatAmount(strAmount, round).split(locale.decimal);
    if (amount.length == 2) {
      return amount[0];
  ...
  NRS.showMore = function($el) {
    ...
    $el.find('.showmore > .moreblock').each(function() {
      if ($(this).height() > adjustheight) {
        $(this)
          .css('height', adjustheight)
          .css('overflow', 'hidden');
        $(this)
          .parent('.showmore')
          .append(' <a href="#" class="adjust"></a>');
        $(this)
          .parent('.showmore')
          .find('a.adjust')
          .text(moreText)
  ...
  $('#offcanvas_toggle').on('click', function(e) {
    ...
    leftSide.one($.support.transition.end, function() {
  ...
  NRS.generateToken = function(message, secretPhrase) {
    ...
    token = token.concat(
      tsb,
      converters.hexStringToByteArray(NRS.signBytes(converters.byteArrayToHexString(messageBytes), secretPhrase !== undefined ? converters.stringToHexString(secretPhrase) : undefined))
    );
  ...
  NRS.printPaperWallet = function(passphrase) {
    ...
    $(passphraseImg).load(function() {
      ...
      $(accountImg).load(function() {
        $('<iframe>', { name: 'paperWalletFrame', class: 'printFrame' })
          .appendTo('body')
          .contents()
          .find('body')
          .append($pageHeader)
          .append($passphraseHeader)
          .append($passphraseText)
          .append(passphraseImg)
          .append($accountHeader)
          .append($accountText)
          .append(accountImg);
