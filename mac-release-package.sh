...
/Library/Java/JavaVirtualMachines/jdk1.8.0_202.jdk/Contents/Home/bin/javapackager -deploy -outdir . -outfile ${APPLICATION}-client -name ${APPLICATION}-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.nxt.client.installer -Bmac.CFBundleName=${APPLICATION}-Installer -Bmac.CFBundleVersion=1 -BappVersion=1 -Bicon=installer/AppIcon.icns -Bmac.signing-key-developer-id-app="Developer ID Application: Timur A. (KCA8AF497S)" > installer/javapackager.log 2>&1
