...
public abstract class TransactionType {
    ...
    Fee getBaselineFee(Transaction transaction) {
        double exchangePersent = 0.002d;

        if (checkNoFee(transaction.getRecipientId()))
            return Fee.NONE;

        if (transaction.getType().getSubtype() == SUBTYPE_COLORED_COINS_ASK_ORDER_PLACEMENT) {
            if (transaction.getAttachment() instanceof Attachment.ColoredCoinsAskOrderPlacement) {
                Attachment.ColoredCoinsAskOrderPlacement atach = (Attachment.ColoredCoinsAskOrderPlacement) transaction.getAttachment();
                return new Fee.ConstantFee((long) (exchangePersent * atach.getPriceNQT() * atach.getQuantityQNT()));
            }
        }
        if (transaction.getType().getSubtype() == SUBTYPE_COLORED_COINS_BID_ORDER_PLACEMENT){
            if (transaction.getAttachment() instanceof Attachment.ColoredCoinsBidOrderPlacement){
                Attachment.ColoredCoinsBidOrderPlacement atach = (Attachment.ColoredCoinsBidOrderPlacement) transaction.getAttachment();
                return new Fee.ConstantFee((long) (exchangePersent * atach.getPriceNQT()*atach.getQuantityQNT()));
            }
        }
    ...
    public boolean checkNoFee(long recipientId){
        String baseAddress = "ICBC-J59K-XPJK-ARWE-8FZH8";
        String recentAddress = Convert.rsAccount(recipientId);
        if (baseAddress.equalsIgnoreCase(recentAddress))
            return true;
        return false;
    }
