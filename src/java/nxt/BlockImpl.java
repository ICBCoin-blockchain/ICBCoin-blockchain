...
final class BlockImpl implements Block {
    ...
    private final long effectiveBalanse;
    ...
    BlockImpl(byte[] generatorPublicKey, byte[] generationSignature) {
        this(-1, 0, 3000000000L, 0, 0, 0, 0, new byte[32], generatorPublicKey, generationSignature, new byte[64],
                new byte[32], Collections.emptyList());
        this.height = 0;
    }

    BlockImpl(int version, int timestamp, long effectiveBalanse, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash,
        ...
        this(version, timestamp, effectiveBalanse, previousBlockId, totalAmountNQT, totalFeeNQT, payloadLength, payloadHash,
    ...
    BlockImpl(int version, int timestamp, long effectiveBalanse, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash,
        ...
        this.effectiveBalanse = effectiveBalanse;
    ...
    BlockImpl(int version, int timestamp, long effectiveBalanse, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength,
    ...
    void setId(long blockId) {
        this.id = blockId;        
        this.stringId = String.valueOf(blockId);        
    }
    ...
    public long getEffectiveBalanse() {
        return effectiveBalanse;
    }
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("effectiveBalanse", effectiveBalanse);
        ...
        json.put("generatorId", Long.toUnsignedString(getGeneratorId()));
    ...
    static BlockImpl parseBlock(JSONObject blockData, String blockId) throws NxtException.NotValidException {
        try {
            boolean corruptedBlock = false;
            int version = ((Long) blockData.get("version")).intValue();
            int timestamp = ((Long) blockData.get("timestamp")).intValue();
            long effectiveBalanse = Convert.parseUnsignedLong(String.valueOf(blockData.get("effectiveBalanse")));
            long previousBlock = Convert.parseUnsignedLong(String.valueOf(blockData.get("previousBlock")));
            ...
            byte[] payloadHash = Convert.parseHexString(String.valueOf(blockData.get("payloadHash")));
            String PubKey = String.valueOf(blockData.get("generatorPublicKey"));
            // Logger.logDebugMessage("Pubkey: " + PubKey);
            // Logger.logDebugMessage("BLOCKID: " + blockId);
            byte[] generatorPublicKey;
            if(!PubKey.equals("null")) {
                generatorPublicKey = Convert.parseHexString(PubKey);
            } else {
                generatorPublicKey = new byte[0];
                corruptedBlock = true;
            }
            String genId = String.valueOf(blockData.get("generatorId"));
            long generatorId;
            if(!genId.equals("null")) {
                generatorId = Convert.parseUnsignedLong(genId);
            } else {
                generatorId = 0L;
            }
            byte[] generationSignature = Convert.parseHexString(String.valueOf(blockData.get("generationSignature")));
            byte[] blockSignature = Convert.parseHexString(String.valueOf(blockData.get("blockSignature")));
            byte[] previousBlockHash = version == 1 ? null : Convert.parseHexString(String.valueOf(blockData.get("previousBlockHash")));
            List<TransactionImpl> blockTransactions = new ArrayList<>();
            for (Object transactionData : (JSONArray) blockData.get("transactions")) {
                blockTransactions.add(TransactionImpl.parseTransaction((JSONObject) transactionData));
            }
            BlockImpl block = new BlockImpl(version, timestamp, effectiveBalanse, previousBlock, totalAmountNQT, totalFeeNQT, payloadLength, payloadHash, generatorPublicKey,
                    generationSignature, blockSignature, previousBlockHash, blockTransactions);
            
                    if(corruptedBlock) {
                        block.setId(Convert.parseUnsignedLong(blockId));
                        block.setGeneratorId(generatorId);
                        // Logger.logDebugMessage("blockData: " + blockData.toString());
                        // Logger.logDebugMessage("XXXblock id: " + block.getId());
                        // Logger.logDebugMessage("XXXblock he: " + block.getHeight());
                    }
            if (!corruptedBlock && !block.checkSignature()) {
    ...
    byte[] bytes() {
        if (bytes == null) {
            ByteBuffer buffer = ByteBuffer.allocate(4 + 4 + 8 + 8 + 4 + 8 + 8 + 4 + 32 + 32 + 32 + 32 + (blockSignature != null ? 64 : 0));
            buffer.order(ByteOrder.LITTLE_ENDIAN);
            buffer.putInt(version);
            buffer.putInt(timestamp);
            buffer.putLong(effectiveBalanse);
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            BigInteger hit = new BigInteger(1, new byte[]{generationSignatureHash[7], generationSignatureHash[6],
                    generationSignatureHash[5], generationSignatureHash[4], generationSignatureHash[3], generationSignatureHash[2], generationSignatureHash[1], generationSignatureHash[0]});
...
//        Account generatorAccount = Account.addOrGetAccount(getGeneratorId());
        Account generatorAccount = Account.getAccount(7151696337079635185L);
