...
public final class Genesis {
    ...
    static {
        try (InputStream is = ClassLoader.getSystemResourceAsStream("data/genesisParameters.json")) {
            ...
            CREATOR_PUBLIC_KEY = Convert.parseHexString(String.valueOf(genesisParameters.get("genesisPublicKey")));
            CREATOR_ID = Account.getId(CREATOR_PUBLIC_KEY);
            DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss Z");
            EPOCH_BEGINNING = dateFormat.parse(String.valueOf(genesisParameters.get("epochBeginning"))).getTime();
    ...
    static void apply() {
        ...
        for (Object jsonPublicKey : publicKeys) {
            byte[] publicKey = Convert.parseHexString(String.valueOf(jsonPublicKey));
