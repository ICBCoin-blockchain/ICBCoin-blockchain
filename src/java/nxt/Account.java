...
public final class Account {
    ...
    public boolean isValidPubKey() {
        return publicKey != null && publicKey.publicKey != null && publicKey.publicKey.length > 0;
    }
    ...
    public long getEffectiveBalanceNXT(int heightBlock) {
...
//        }
       ...
       if (this.publicKey == null || this.publicKey.publicKey == null) {
        ...
        Block block = Nxt.getBlockchain().getBlockAtHeight(heightBlock);
        long defaultEfectBalanse = block.getEffectiveBalanse();
        if (heightBlock < 16)
            return defaultEfectBalanse;
        Block lastBlock = Nxt.getBlockchain().getBlockAtHeight(heightBlock-16);
        long deltaTime = (block.getTimestamp() - lastBlock.getTimestamp())/16;// in second
        float coefDeltaBalanse = calculateKoefDeltaBalanse(deltaTime);
        defaultEfectBalanse = defaultEfectBalanse + (long)(defaultEfectBalanse * coefDeltaBalanse);
        return defaultEfectBalanse;
    }

    private float calculateKoefDeltaBalanse(long deltaTime){
        if (deltaTime <= 10)
            return -0.4f;
        if (deltaTime <= 20)
            return -0.3f;
