...
final class PeerImpl implements Peer {
    ...
    public String getSoftware() {
        return Convert.truncate(application, "?", 10, false) + " (" + Convert.truncate(version, "?", 10, false) + ")"
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof NxtException.NotCurrentlyValidException
            ...
            // don't blacklist peers just because a feature is not yet enabled, or because
            // of database timeouts
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket) {
                ...
                // Logger.logMessage("wsrequest: " + wsRequest + " wsresponse: " + wsResponse + "\n");
            ...
            if (!(e instanceof UnknownHostException || e instanceof SocketTimeoutException
                ...
                Logger.logDebugMessage(String.format("Error sending request to peer %s: %s", host,
        ...
        // Logger.logMessage("request: " + JSON.toString(request) + " response: " + response + "\n");
        return response;
    }
            ...
            if (!Peers.ignorePeerAnnouncedAddress && announcedAddress != null) {
                try {
                    ...
                    if (!inetAddress.equals(InetAddress.getByName(host))) {
                        Logger.logDebugMessage("Connect: announced address " + announcedAddress + " now points to "
                                + inetAddress.getHostAddress() + ", replacing peer " + host);
                        Peers.removePeer(this);
                    ...
                    if (newAnnouncedAddress != null) {
                        ...
                        if (newAnnouncedAddress != null) {
                            if (!verifyAnnouncedAddress(newAnnouncedAddress)) {
                                ...
                                if (!verifyAnnouncedAddress(announcedAddress)) {
                                    Logger.logDebugMessage(
                                            "Connect: old announced address for " + host + " no longer valid");
                            ...
                            if (!newAnnouncedAddress.equals(announcedAddress)) {
                                Logger.logDebugMessage("Connect: peer " + host + " has new announced address "
                                        + newAnnouncedAddress + ", old is " + announcedAddress);
                                int oldPort = getPort();
        ...
        try {
            ...
            if (hallmark != null && announcedPort != hallmark.getPort()) {
                Logger.logDebugMessage("Announced port " + announcedPort + " does not match hallmark "
                        + hallmark.getPort() + ", ignoring hallmark for " + host);
                unsetHallmark();
            }
    ...
    public boolean isApiConnectable() {
        return isOpenAPI() && state == Peer.State.CONNECTED && !Peers.isOldVersion(version, Constants.MIN_PROXY_VERSION)
                && !Peers.isNewVersion(version) && blockchainState == Peer.BlockchainState.UP_TO_DATE;
    }
    ...
    public String toString() {
        return "Peer{" + "state=" + state + ", announcedAddress='" + announcedAddress + '\'' + ", services=" + services
